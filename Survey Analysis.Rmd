...
<font size = "5">Hi if you participated in this survey, you will be interested to see some results. We are excited to share with you some interesting insights!
...
prep_swedish_interest <-
  data.frame(points = rep(1, 4), interest = buckets)

prep_swedish_interest %>%
  ggplot(aes(x = points, fill = interest)) +
  geom_bar(position = "stack", show.legend = FALSE, width = .4) +
  geom_text(aes(label = buckets,
                y = seq(
                  from = 0.5, to = 3.5, by = 1
                ))) +
  geom_point(
    aes(x = .75, y = (average_interest_swedish/2) * 4),
    shape = 24,
    size = 7,
    colour = "black",
    fill = "black"
...
prep_swedish_interest %>%
  ...
  geom_point(
    aes(x = .75, y = (average_interest_uppland/2) * 4),
    shape = 24,
    size = 7,
    colour = "black",
    fill = "black"
...
`r str_to_sentence(as.english(survey_feedback_count))` respondents gave feedback on the survey itself, though the data analyst has some to add! The ANA greatly appreciates all the feedback given and we plan to incorporate your suggestions in next year's survey!
